  ...
  if (mfaToken) {
    return (
      <div className="auth-card">
        ...
        <button className="btn-primary auth-btn" onClick={handleOtp} disabled={loading}>
          {loading ? 'Verifying...' : 'Verify'}
        </button>
        <p className="auth-switch">
          <button className="link-btn" onClick={() => { setMfaToken(null); setOtp(''); setError(null) }}>
            ← Back
          </button>
        </p>
      </div>
    ...
    <div className="auth-card">
      ...
      <div className="auth-fields">
        <input className="form-input" type="email" placeholder="Email"
          value={form.email} onChange={set('email')} onKeyDown={e => e.key === 'Enter' && handleSubmit()} autoFocus />
